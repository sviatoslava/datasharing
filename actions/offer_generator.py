...
OFFER_VALUE_TABLE = {
    ...
    ("premium", "high_risk"):   ("cashback", "TBD", 60),
    ("premium", "medium_risk"): ("cashback", "TBD", 30),
    ("premium", "low_risk"):    ("points_multiplier", "TBD", 30),
    ("standard", "high_risk"):  ("cashback", "TBD", 60),
    ("standard", "medium_risk"):("cashback", "TBD", 30),
    ("standard", "low_risk"):   ("points_multiplier", "TBD", 30),
    ("student", "high_risk"):   ("statement_credit", "TBD", 30),
    ("student", "medium_risk"): ("statement_credit", "TBD", 30),
    ("student", "low_risk"):    ("bonus_points", "TBD", 30),
}
...
def _render_offer_action(
    ...
    template = templates.get(offer_category, templates.get("general", "make_your_first_purchase_with_your_card"))
    try:
        rendered = template.format(window=offer_duration_days, n_times=n_times)
    except (KeyError, ValueError):
        rendered = f"use_your_card_for_{offer_category}_in_{offer_duration_days}d"
    return rendered
...
def generate_offers(
    ...
    for _, row in merged.iterrows():
        ...
        specific_action = _render_offer_action(
            offer_category, risk_band, offer_duration, templates,
        )
